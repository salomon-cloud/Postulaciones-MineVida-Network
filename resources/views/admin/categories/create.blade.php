<x-layouts.admin :title="'Nueva categoria | '.config('app.name', 'MineVida Network')">
    ...
    <form class="lumoryx-panel mt-6 d-grid gap-5 p-5 sm:p-6 grid-cols-lg-2" method="POST" action="{{ route('admin.categories.store') }}" x-data="{ accentColor: '{{ old('accent_color', '#facc15') }}' }">
        @csrf
        ...
        <div class="col-span-lg-2 mt-2 border-t border-white/10 pt-5">
        ...
        <label class="col-span-lg-2 d-flex flex-column gap-4 rounded-lg border border-white/10 bg-white/[.035] p-4 text-sm text-slate-300 transition hover:border-amber-300/20 flex-sm-row align-items-sm-center justify-content-sm-between">
            <span>
                <span class="d-block font-semibold text-white">Categoria abierta</span>
                <span class="mt-1 d-block text-slate-400">Si esta activa, los usuarios podran enviar postulaciones a esta categoria.</span>
            </span>
            <input class="lumoryx-toggle-checkbox flex-shrink-0" type="checkbox" name="is_open" value="1" @checked(old('is_open', true))>
        </label>

        <div class="col-span-lg-2 d-grid gap-4 grid-cols-lg-2">
        ...
        <div class="col-span-lg-2 mt-2 rounded-lg border border-white/10 bg-white/[.025] p-4">
            ...
            <div class="mt-4 d-grid gap-4 grid-cols-lg-2">
                @php
        ...
        <div class="col-span-lg-2 d-flex justify-content-end">
