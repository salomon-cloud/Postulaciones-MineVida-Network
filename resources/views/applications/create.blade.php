    ...
    <section class="lumoryx-panel mt-8 p-4 sm:p-6">
        <div class="lumoryx-category-grid">
            @forelse ($types as $item)
                <article class="lumoryx-choice-card group position-relative d-flex h-100 flex-column overflow-hidden {{ ($item['is_open'] ?? true) ? '' : 'opacity-80' }}">
                    ...
                    <div class="lumoryx-category-card-body">
                        <div class="d-flex min-w-0 align-items-start justify-content-between gap-4">
                            <div class="min-w-0">
                        ...
                        @if ($item['is_open'] ?? true)
                            <div class="mt-auto pt-6">
                                <x-lumoryx.button class="w-100" :href="route('applications.create.type', $item['type'])">Postularme</x-lumoryx.button>
                            </div>
                        @else
                            <div class="mt-auto pt-6">
                                <div class="rounded-lg border border-rose-300/20 bg-rose-300/10 p-4">
                                    ...
                                    <x-lumoryx.button class="mt-3 w-100" variant="secondary" :href="route('applications.create.type', $item['type'])">Ver informacion</x-lumoryx.button>
                                </div>
                            </div>
                        @endif
                    </div>
            ...
            @empty
                <div class="col-span-sm-2 col-span-xxl-3">
                    <x-lumoryx.empty-state title="Postulaciones cerradas" body="No hay categorias abiertas por ahora. Vuelve a revisar mas tarde." />
                </div>
            @endforelse
        </div>

        <div class="mt-6 d-flex flex-column gap-4 rounded-lg border border-white/10 bg-white/[.035] p-5 flex-sm-row align-items-sm-center justify-content-sm-between">
            ...
            <x-lumoryx.button class="flex-shrink-0" variant="secondary" href="{{ route('rules') }}">Ver informacion</x-lumoryx.button>
        </div>
    </section>
</x-layouts.user>
